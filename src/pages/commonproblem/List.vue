    		...
    		<table class="common-table">
    			...
    			<tbody>
	    			<tr v-for="(list,index) in lists" :key="list.id">
	    				...
	    				<td>{{index+1}}</td>
	    				<td  class="summary">{{list.problemDescription}}</td>
...
export default {
	...
	methods: {
		//显示列表
		showLists: function() {
			this.$axios.get("/commonproblem").then((response) => {
		...
		commonProblemAdd: function() {
			this.$router.push({name:"commonProblemAdd"})
		},
